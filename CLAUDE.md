...
3. **질의** — 세션 생성과 실행이 분리되어 있다. `POST /api/sessions`는 `status='PENDING'` 행만 만들고 즉시 반환하고, `GET /api/sessions/{id}/stream`(SSE)에 연결하는 시점에 원자적 CAS(`UPDATE ... WHERE status='PENDING'`)로 선점한 연결만 `repoview/agent/loop.py:run_session`을 백그라운드 스레드로 트리거한다(같은 세션에 여러 번 재연결해도 실행은 한 번만 됨). 이 스트림 연결은 이미 스텝마다 즉시 commit되는 `trace_step`을 0.2초 간격으로 폴링해 SSE 이벤트(`step_started`/`step_completed`/`assistant_message`/`done`/`error`)로 변환한다 — `docs/04-api-spec.md` 2절 "구현 노트", `docs/05-agent-design.md` §8 참고. `run_session` 자체는 `agent/prompts.py`로 시스템 프롬프트를 빌드한다 — 레포 개요(언어/프레임워크 + 깊이 2 트리 + 루트 파일명)는 도구로 노출하는 대신 프롬프트 앞부분에 미리 주입해서, 모든 질문이 이를 공짜로 얻고 고정된 접두부가 캐싱 대상이 되게 한다. 이후 루프는 모델이 순수 텍스트를 반환하거나 `MAX_ITERATIONS`(`config.py`, 기본값 6) 또는 누적 토큰 상한(`MAX_SESSION_TOKENS`, 기본 50000)에 도달할 때까지 LLM 호출과 도구 디스패치(`repoview/tools/__init__.py:dispatch`)를 번갈아 수행한다 — 동일 `(tool_name, args)`가 3회 이상 반복되면 결과에 "다른 접근을 시도하라"는 안내를 주입한다(Phase 4). 세션 종료 시 `repoview/citation_check.py:verify_citations`가 리뷰의 `파일:라인` 인용을 사후검증해(존재하지 않는 파일/범위 밖 라인/미확인 인용) 그 결과를 `session.citation_warnings`와 `status`/`final_review`를 **같은 UPDATE/트랜잭션**으로 묶어 기록한다 — 검증 자체가 실패해도(예: 마이그레이션 누락) status 커밋은 항상 뒤따르고, 두 값이 항상 한 번에 반영되므로 SSE 폴링이 상태만 반영된 순간을 관측하는 레이스가 없다(처음엔 두 UPDATE로 분리했다가 바로 이 레이스가 발견돼 하나로 합쳤다). 모든 LLM 호출과 도구 호출은 `GET /api/sessions/{id}`로 재생할 수 있도록 `trace_step`에 기록된다.
