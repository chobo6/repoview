...
`run_session`은 이미 스텝마다 `trace_step`에 즉시 commit하므로, 이 커밋 자체를 SSE 이벤트 소스로 재사용한다(`docs/04-api-spec.md` §2 "구현 노트" 참고) — 루프 구조 자체는 바꾸지 않는다.

**`run_session`의 유일한 변경**: `session_id: int | None = None` 파라미터 추가. 주어지면 `_create_session` 호출(새 행 INSERT)을 건너뛰고 그 ID를 그대로 사용한다. `session_id`가 없을 때의 기존 동작(테스트, `eval.py`)은 그대로다 — 이 둘은 세션 생성과 실행이 분리될 필요가 없어 계속 한 번에 처리한다.
